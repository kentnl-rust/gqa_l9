...
impl Gen2i {
    ...
    fn nextj(&mut self) -> (u32, u32) {
        ...
        self.idx = (self.idx + 1) % 128; // per_kib!(u64);
        if self.idx == 0 {
            self.more();
        }
        rv
...
mod kat_tests {
    ...
    fn compare_kats(fexpected: &str, variant: super::Variant) {
        ...
        rv = rv + &h0output + &blockoutput + &u8info("Tag", &out, false);
